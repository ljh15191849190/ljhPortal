        ...
        div.filter-tree(v-show="isShowTree")
            ...
            @node-click="treeNodeOnClick")
                span(slot-scope="{node, data}")
                    i.theme-color.portal-icon-organization_tree
                    span {{node.label}}
...
<script>
    ...
    export default {
        ...
        props: {
            ...
            projectId: {
                type: String,
                // props default在组件注册的时候就固定了值，并不是组件实例的初始化
        ...
        computed: {
            projectIdForReq () {
                return this.projectId || localStorage.getItem('tenant')
            }
        },
        methods: {
            handleClickOutSide () {
            ...
            inputChange (customInput) {
                ...
                this.inputChangeTimer = setTimeout(() => {
                    if (customInput !== '') {
                        Api.queryOrgByNameOrId({
                            org_name: customInput,
                            project_id: this.projectIdForReq
                        }).then(res => {
            ...
            getOrgByParent (node, resolve) {
                ...
                Api.getOrgByParent(parentId, this.projectIdForReq).then(res => {
                    resolve(res.orgs)
                })
            },
